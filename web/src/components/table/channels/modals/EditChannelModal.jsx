...
const MODEL_FETCHABLE_TYPES = new Set([
  1, 58, 59, 4, 14, 34, 17, 26, 27, 24, 47, 25, 20, 23, 31, 40, 42, 48, 43,
]);
const OPENAI_COMPAT_CHANNEL_TYPES = new Set([1, 58, 59]);
...
const EditChannelModal = (props) => {
  ...
  const submit = async () => {
    ...
    // OpenAI / Claude: 设置字段透传控制（显式保存布尔值）
    if (OPENAI_COMPAT_CHANNEL_TYPES.has(localInputs.type) || localInputs.type === 14) {
      settings.allow_service_tier = localInputs.allow_service_tier === true;
      // 仅 OpenAI 渠道需要 store / safety_identifier / include_obfuscation
      if (OPENAI_COMPAT_CHANNEL_TYPES.has(localInputs.type)) {
            ...
            const advancedSettingsContent = (
              <div className='space-y-4'>
                ...
                <div className='py-3 border-b border-gray-100'>
                  ...
                  {OPENAI_COMPAT_CHANNEL_TYPES.has(inputs.type) && (
                    <>
